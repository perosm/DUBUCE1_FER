...
class FC(Layer):
  ...
  def forward(self, inputs):
    ...
    """
    # TODO
    self.inputs = inputs # needed for parameter update in backward_params
    # (N, num_inputs) x (num_outputs, num_inputs).T + (num_outputs)
  ...
  def backward_inputs(self, grads):
    ...
    """
    # TODO
    # (N, num_outputs) x (num_outputs, num_inputs)
  ...
  def backward_params(self, grads):
    ...
    """
    # TODO
    # (N, num_outputs).T x (N, num_inputs)
...
class ReLU(Layer):
  ...
  def backward_inputs(self, grads):
    ...
    """
    # TODO
    return np.where(self.inputs < 0, 0, 1) * grads
...
class SoftmaxCrossEntropyWithLogits():
  ...
  def forward(self, x, y):
    ...
    #return - np.mean(np.log(softmax[np.arange(0, N), y_true]))
    first_part = np.log(np.sum(np.exp(x), axis=1))
    second_part = np.sum(x * y, axis=1)
  ...
  def backward_inputs(self, x, y):
    ...
    softmax = numerator / denominator
    #y_true = np.argmax(y)
    #softmax[np.arange(0, N), y_true] -= 1
    #return softmax / N
    return (softmax - y) / N
